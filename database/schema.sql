...
COMMENT ON COLUMN HOLIDAYS.updated_by IS 'User who last updated record';
  ...
  CREATE TABLE GOALS (
    ...
    CONSTRAINT fk_goal_emp FOREIGN KEY (employee_id)
...
CREATE INDEX idx_goal_deadline ON GOALS(deadline);
    ...
    CREATE OR REPLACE TRIGGER trg_goal_deadline_validation
BEFORE INSERT OR UPDATE ON GOALS
FOR EACH ROW
BEGIN
    IF :NEW.deadline IS NOT NULL AND :NEW.deadline < TRUNC(SYSDATE) THEN
