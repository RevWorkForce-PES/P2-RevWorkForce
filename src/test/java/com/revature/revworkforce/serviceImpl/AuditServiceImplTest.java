...
class AuditServiceImplTest {
    ...
    void createAuditLog_WithEmployee() {
        ...
        AuditLog result = auditService.createAuditLog("EMP001", "ACTION", "TABLE", "1", "OLD", "NEW", "IP", "UA");
    ...
    void createAuditLog_System() {
        ...
        AuditLog result = auditService.createAuditLog("SYSTEM", "ACTION", "TABLE", "1", "OLD", "NEW", "IP", "UA");
    ...
    void getRecentAuditLogs_Success() {
        when(auditLogRepository.findRecentLogs(10)).thenReturn(Arrays.asList(auditLog));
    ...
    @Test
    void getAuditLogsByTableAndRecord_Success() {
        when(auditLogRepository.findByTableNameAndRecordIdOrderByCreatedAtDesc("TABLE", "1"))
                .thenReturn(Arrays.asList(auditLog));

        List<AuditLog> result = auditService.getAuditLogsByTableAndRecord("TABLE", "1");
    ...
    void getAuditLogsByDateRange_Success() {
        ...
        when(auditLogRepository.findByDateRange(any(), any())).thenReturn(Arrays.asList(auditLog));

        List<AuditLog> result = auditService.getAuditLogsByDateRange(now.minusDays(1), now);
    ...
    void getLoginActivities_Success() {
        when(auditLogRepository.findByAction(Constants.AUDIT_LOGIN)).thenReturn(Arrays.asList(auditLog));
    ...
    void cleanOldAuditLogs_Success() {
        when(auditLogRepository.deleteOldLogs(any())).thenReturn(5);
    ...
    void logAction_Success() {
        ...
        auditService.logAction("EMP001", AuditAction.UPDATE, "ENTITY", "1", "Details", "IP");
    ...
    void searchAuditLogs_Success() {
        when(auditLogRepository.searchLogs(any(), any(), any(), any())).thenReturn(Arrays.asList(auditLog));

        List<AuditLogDTO> result = auditService.searchAuditLogs("All Modules", null, null, "keyword");
    ...
    void exportAuditLogsToCSV_Success() {
        ...
        assertThat(csv).contains("Log ID,Timestamp,User,Action,Module,Details");
